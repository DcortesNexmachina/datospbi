...
class CustomerFlowAnalysis:
    ...
    def create_visualizations(self, output_dir: Path, df_store) -> None:
        ...
        period1_end = datetime(2024, 11, 13)
        period2_start = datetime(2024, 11, 14)
        period2_end = datetime(2024, 12, 6)

        plt.figure(figsize=(12, 6))
        self.df.set_index('Fecha')['Entradas'].plot()
        plt.axvspan(period2_start, period2_end, color='blue', alpha=0.3, label='Durante Euskal Bono Denda')
        ...
        sns.boxplot(data=[self.period1_data, self.period2_data])
        plt.title(f'Distribución de Entradas por Período - {self.store_name}')
    ...
    def generate_html_report(self, stats: Dict, output_dir: Path, campaign_name: str) -> None:
        # Determine results based on statistical tests
        ...
        <body>
            ...
            <div class="narrative">
                ...
                <ul>
                    <li><strong>Antes de la campaña {{ campaign_name }}</strong> (periodo inicial)</li>
                    <li><strong>Durante la campaña {{ campaign_name }}</strong> (cuando la campaña estuvo activa)</li>
                </ul>

                <h3>Análisis de las Métricas Principales</h3>
                <p>
                    El análisis reveló que antes de la campaña, la media de entradas fue de {{ "%.2f"|format(stats.period1_mean) }} 
                    clientes por hora ({{ "%.2f"|format(stats.period1_mean*8) }} clientes por dia). Durante la campaña, esta cifra {{ "aumentó" if stats.period2_mean > stats.period1_mean else "disminuyó" }} 
                ...
                <p>
                    ...
                    hubo fluctuaciones en el número de entradas durante la campaña.
                    {% else %}
                    el patrón de entradas fue más estable durante la campaña.
                    {% endif %}
                </p>
                ...
                Similarmente, para el periodo durante la campaña, 
...
<p>
    ...
    {% endif %}
                ...
                <div class="visualization">
                    ...
                    <p>El gráfico muestra la evolución temporal de las entradas, con la zona sombreada 
                    indicando el periodo durante la campaña {{ campaign_name }}.</p>
                </div>
...
def analyze_store(df_store: pd.DataFrame, store_name: str, base_output_dir: Path):
    try:
        ...
        analysis.generate_html_report(stats, store_output_dir, "Euskal Bono Denda")
...
def main():
    try:
        ...
        base_output_dir = Path('informes_comercios_ebdenda')
